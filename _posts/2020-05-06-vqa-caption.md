...
This seems impossible, especially when there is no additional training involved. How can an image captioner that is only trained on image caption dataset be sensitive to questions from VQA dataset? To answer this question, we need to rethink what question is and introduce Rational Speech Act (RSA)  framework. A more technical overview of this is described 

Previous works that studied the image captioning setting (and used methods somewhat similar to ours) mostly focused on the concept of "granularity" -- how detailed can the text be [^3][^4][^5]. Controlling granularity is important but is orthogonal to our focus. The work described by this post is my new paper with Reuben Cohn-Gordon and Chris Potts[^6].
...
Before we show what RSA is, we should
...
## RSA + VQA: Is Our Caption Issue-Aware?
