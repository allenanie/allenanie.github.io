...
Despite such an impressive result, controlling generative models in text has been far more difficult. Unlike images, where humans have an intuitive idea on what to control: rotation angle, brightness, X-Y axis displacement, etc., it is far more difficult to control for text beyond simple properties like sentiment or tense (Hu et al.[^1]). Later work such as Lample et al.[^2] leveraged the domain where the data is collected to add additional attributes like gender, race, age, and interests (like music, book, movie, etc.), and trained a neural network from scratch conditioned on these attributes. However, for vagueness, politeness, genericness, sarcasm, humor, formality, pedantic vs. informal, we have made little progress in developing neural network based decoding method to control for these aspects of text. 

In this post, we will focus on one aspect: Question-under-Discussion (QuD). QuD is a formal linguistic model of discourse, first proposed in 1984. Informally, QuD Model suggests that for every sentence we write, there is a corresponding underlying question. For example, if I write "She ate the pasta.", the underyling QuD that made me write this sentence is "What did she eat?". QuD Model turns any paragraph of text into a conversation in the writer's head. 
...
The computational procedure is that we first normalize the columns of $S_0$, to turn it from a row-stochastic matrix to a column-stochastic matrix, arriving at $L_1$, and then normalize again to turn $L_1$ back to a row-stochastic matrix $S_1$. This process captures how the model considering "reasonable" alternatives of its choices and then decide what to select to achieve its best unambiguous outcome. We can see in bold number that $S_1$ will choose `baseball cap` to describe the first image, contrasting the other two.
...
Equipped with the knowledge of RSA, now the question becomes: how do we pick out a set of images such that for the target image, the caption we produced will select the feature that is the answer to the question?
